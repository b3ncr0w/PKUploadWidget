...
const monitorProcessing = (jobId, callback) => {
...
const monitorPublishing = (appSecret, callback) => {
  return new Promise((resolve) => {
    const interval = setInterval(() => {
...
export const process = async ({ platform, uploadId }, callbacks, state) => {
  ...
  await monitorProcessing(processData.job_guid, (percentage, message) => {
    if (state.isCancelled) return;
  ...
  await monitorPublishing(newAppData.app_secret, (percentage, message) => {
    if (state.isCancelled) return;
